...
async def validate_onboarding_answer(
    ...
    system = (
        ...
        "Правила:\n"
        "1. Если ответ адекватный — верни нормализованный текст.\n"
        "2. Нормализация: убери мусор, оставь суть, сделай грамотно и кратко "
        "(1-2 предложения макс).\n"
        "3. ОБЯЗАТЕЛЬНО включи контекст вопроса в нормализованный текст, "
        "чтобы ответ был понятен БЕЗ вопроса. "
        'Пример: вопрос "Как засыпает?", ответ "на руках" → '
        '"Засыпает на руках".\n'
        "4. НЕ изменяй имена собственные — сохраняй их ТОЧНО как написано.\n"
        "5. Если ответ явно невалидный (невозможные данные для возраста, бессмыслица, "
        "не относится к ребёнку) — укажи это.\n"
        "6. Если пользователь написал 'не знаю', 'пропустить', 'хз', 'потом' — "
        "это пропуск.\n\n"
        "Формат ответа строго JSON:\n"
        '{"status": "ok", "text": "нормализованный текст с контекстом вопроса"}\n'
