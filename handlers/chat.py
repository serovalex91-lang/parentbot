...
async def handle_chat(message: Message, bot: Bot, config: Config = None, db_user: dict = None):
    ...
    # Параллельно: RAG + история + исключения + Brave + контекст ребёнка
    search_mode = db_user.get("search_mode", "kb_only")
    max_hist = config.max_history_messages if config else 20

    excluded_ids_task = db.get_excluded_book_ids(user_id)
    history_task = db.get_last_messages(user_id, limit=max_hist)
    child_context_task = _get_child_context_str(db_user)

    excluded_ids, history, child_context = await asyncio.gather(
        excluded_ids_task, history_task, child_context_task
    )
    ...
    # Brave Search (если режим kb_internet) — параллельно с RAG не можем т.к. нужен typing
    brave_text = ""
    ...
    # Валидация истории: Claude требует чередование user/assistant
...
def _validate_history(history: list) -> list:
    ...
    for msg in history[1:]:
        if msg["role"] != validated[-1]["role"]:
            ...
            # Объединяем если две подряд одной роли
            validated[-1]["content"] += "\n" + msg["content"]
    return validated
